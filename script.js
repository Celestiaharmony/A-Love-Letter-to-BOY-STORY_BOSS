...
function updateWallStatus() {
  ...
  const span =
    statusElement.querySelector(`[data-lang="${currentLang}"]`);
  ...
  span.textContent =
    approvedMessages.length === 0
      ...
      : statusText.shown[currentLang](approvedMessages.length);
}
...
function openFanDialog(row) {
  ...
  nickname.textContent = row["Nickname"] || "Anonymous";
  ...
  const metaValues = [
    row["Country of origin"] ? { icon: "🌍", text: row["Country of origin"] } : null,
    row["Favorite member"] ? { icon: "❤️", text: row["Favorite member"] } : null
  ].filter(Boolean);
  ...
  if (
    String(row.publish_photo || "").trim().toLowerCase() === "yes" &&
    row["Upload photo"]
  ) {
    ...
    image.alt = `Photo from ${row["Nickname"] || "fan"}`;
  ...
  addDialogSection(content, "How they first knew BOY STORY",
    row["When and how did you first get to know BOY STORY?"], "✨");
  addDialogSection(content, "Favorite moment or song",
    row["Favorite moment or song"], "🎵");
  addDialogSection(content, "Message for BOY STORY",
    row["Message for BOY STORY"], "💌", "quote");
  addDialogSection(content, "Message for certain members",
    row["Message for certain members"], "💬");
  addDialogSection(content, "Free extras",
    row["Free extras"], "🎁");

  dialog.showModal();

  const fanPaper = document.querySelector(".fan-paper");
  if (fanPaper) {
...
function addDialogSection(container, title, text, icon, variant) {
  ...
  section.className = variant ? `dialog-section dialog-section--${variant}` : "dialog-section";
...
function initFanDialog() {
  const dialog = document.getElementById("fan-dialog");
  const closeButton = document.querySelector(".fan-dialog-close");
...
async function loadFanWall() {
  ...
  try {
    ...
    const cardThemes = ["theme-blue", "theme-pink", "theme-lavender", "theme-blush", "theme-skyline", "theme-white"];

    approvedMessages.forEach((row, index) => {
      const card = document.createElement("article");
      card.className = `fan-card ${cardThemes[index % cardThemes.length]}`;
      ...
      country.textContent = row["Country of origin"] || "Unknown country";

      const member = document.createElement("div");
      member.className = "member";
      member.textContent = row["Favorite member"]
        ? `Favorite member: ${row["Favorite member"]}`
        : "";

      const message = document.createElement("div");
      message.className = "message";
      message.textContent = `"${row["Message for BOY STORY"] || "No message."}"`;

      const favorite = document.createElement("div");
      favorite.className = "song";
      favorite.textContent = row["Favorite moment or song"]
    ...
    if (statusElement) {
      const span = statusElement.querySelector(`[data-lang="${currentLang}"]`);
      if (span) span.textContent = statusText.error[currentLang];
    }
  }
}
